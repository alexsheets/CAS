    ...
    public class AdminViewController : Controller
    {
        ...
        public void ProcessScores(DateTime start, DateTime end, string year)
        {
            ...
            foreach(long id in studentIds)
            {
                ...
                {
                    // there is an error with one of the students missing who did EPAs
                    // likely means that someone created an account and did some tests then deleted the account? i dont know
                    continue;
                }
        ...
        public void ProcessSpecificStudentScoresSelfEvals(DateTime start, DateTime end, string year)
        {
            ...
            foreach (Student student in students)
            {
                ...
                if (numOfSelfEvals > 0)
                {
                    ...
                    {
                        report.Reflection1 = "N/A";
                        report.Reflection2 = "N/A";
                        report.Reflection3 = "N/A";
                    }
        ...
        [HttpPost]
        public string ApproveExternship(int externshipId)
        {
            try
            {
                if (_userInfo.IsValidUser())
                {
                    ...
                    if (externship != null)
                    {
                        ...
                        if (externship.Approved == 1)
                        {
                            TempData["ValidationMsg"] = "This externship has already been approved.";
                            return "Reload";
                        }
                        
                        string externship_response = _setDbItems.ApproveExternship(externship);

                        if (externship_response == "SUCCESS!")
                        {
                            ...
                            string newPass = new(chars);

                            // create evaluator according to the information passed in
                            Evaluator evaluator = new Evaluator();
                            evaluator.Email = externship.EvaluatorEmail;
                            evaluator.FullName = externship.EvaluatorName;
                            evaluator.Lsuind = false;
                            evaluator.Clinic = externship.NameOfPractice;
                            evaluator.Password = newPass;
                            evaluator.UpdateDt = DateTime.Now;
                            evaluator.Active = true;

                            string evaluator_response = _setDbItems.CreateEvaluator(evaluator);

                            if (evaluator_response == "SUCCESS!")
                            {
                                // send email to the externship evaluator
                                SendEmailNonLSU(externship.EvaluatorEmail, newPass);
                                TempData["ValidationMsg"] = "Successfully approved externship and created evaluator.";
                                return "Externship was approved";
                            } else
                            {
                                TempData["ValidationMsg"] = "There was an error creating the new evaluator.";
                                return "Reload";
                            }
                        } else
                        {
                            TempData["ValidationMsg"] = "There was an error approving the externship.";
                            return "Reload";
                        }
                    }
                    else
        ...
        public ActionResult SendEmailNonLSU(string email, string password)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    body = body + "Please note, you will also need to complete an In-Training Evaluation Report (ITER) for this student through a different system, called E-value. You will receive separate communications regarding how to complete the ITER and access the E-value system. ";
                    var smtp = new SmtpClient
                    {
                        Host = "relay.lsu.edu",
                        Port = 25,
        ...
        public virtual JsonResult ReadExternships([DataSourceRequest] DataSourceRequest dataSourceRequest)
        {
            ...
            List<ExternshipRegistration> externships = _getDbItems.GetExternshipRegistrations().Where(e => e.Visibility == 1).ToList();
