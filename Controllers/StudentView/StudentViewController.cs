        ...
        public ActionResult SendEmail(string email, string request)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    var body = "An EPA has been requested for your evaluation. You should now be able to log in and see it at cas.vetmed.lsu.edu \r\n";
                    body = body + "If the student has requested to attach a special request, it will be here: " + request;
            ...
            try
            {
                if (_userInfo.IsValidUser())
                {
                    ...
                    if (comments != null)
                    {
                        ...
                        HttpContext.Session.SetString("comment3", comments.Comment3);
            ...
            if (_userInfo.IsValidUser())
            {
                ...
                if (self_evaluation == false)
                {
                    ...
                    if (result == "SUCCESS!")
                    {
                        ...
                        {
                            if (requested_epa.RequestToEvaluator != null)
                            {
                                SendEmail(evtr.Email, requested_epa.RequestToEvaluator);
                            } else
                            {
